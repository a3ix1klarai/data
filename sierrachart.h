...
struct s_sc
{
	...
	int BarIndexToRelativeHorizontalCoordinate(int BarIndex)
	{
		...
		if (BarIndex > IndexOfLastVisibleBar)
			return static_cast<int>(CHART_DRAWING_MAX_HORIZONTAL_AXIS_RELATIVE_POSITION);
		...
		return static_cast<int>(Percent * CHART_DRAWING_MAX_HORIZONTAL_AXIS_RELATIVE_POSITION);
	...
	*/
	...
	// 2497
	SCDateTimeMS (SCDLLCALL* ConvertDateTimeUTCToChartTimeZone)(const SCDateTime& DateTime);
	...
	//Graphics
	//int(SCDLLCALL* Graphics_LineTo(const int XCoordinate, const int YCoordinate);
